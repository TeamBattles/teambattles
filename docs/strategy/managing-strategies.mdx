...
- **Tokens per side match the game.** Black Ops 6 and Black Ops 7 both use 4 players per side, so the canvas opens with 8 tokens in two compact grids (4 blue, 4 red). For Black Ops 7 this is the competitive team size - casual matches are 6v6, but the strategy editor always plans at the competitive size regardless of which team size a match uses. Counter-Strike 2 and League of Legends use 5 players per side, so those maps open with 10 tokens (5 blue, 5 red).
...
<Frame>
	<img
		src="/tools/battles-strategy/preview-3.webp"
		alt="Battles Strategy editor with frame, animation, and project export choices"
	/>
</Frame>

### Branded frame on image exports

The Export Frame dialog has a **Branded frame** switch. Turn it on and the exported image is mounted
in the TeamBattles look - the hex-mesh honeycomb ground, one accent rail along the top edge, the
crossed-swords wordmark, your board's name and the frame you picked - so a board posted into a
channel says what it is without its filename. The frame is skinned from your own **Appearance**
settings, so a board exported on the Light theme or the cyan accent matches the app you are looking
at.

It is off by default, because a strategy frame is most often dropped straight into a callout or a
stream overlay where a header band is in the way. The frame paints a solid background, so turning it
on disables **Transparent background** for that export.

New boards also start on your theme's own background color instead of a fixed near-black. A board
you have already saved keeps whatever background it was given, and you can change it any time from
the canvas settings.

### Watermark on image and animation exports

Frame and animation exports (PNG, JPEG, WebP, MP4, WebM, GIF) carry a small `teambattles.gg` wordmark in the bottom-right corner on Free and Basic accounts, and on exports made while signed out. Standard and Premium export without it.

The mark never covers your drawing area edge-on: it sits in the corner at a fixed inset and scales with the export resolution, so a 3x export gets a proportionally sized mark rather than a tiny one. Project files (`.tbstrat`) are never watermarked, at any tier, because they are editable source rather than a finished image.
...
<Warning>
	Project files do not carry image shapes. If a file names any, they are left out of the import and you are told how many were skipped. Everything else in the file, including the custom map, imports normally.
</Warning>
...
If your account copy also changed since this device last saw it - because you edited the strategy on another device, or a live session moved the board - the prompt changes to **Two different versions of this board** and you have to choose before you can keep working. You cannot dismiss it, and auto-save is paused while it is open.

<Steps>
	<Step title="Keep this device's version">
		Uploads what is on this device and overwrites the account copy.
	</Step>
	<Step title="Keep the account version">
		Keeps what is already on your account and drops this device's changes.
	</Step>
</Steps>

<Warning>
	Whichever version you do not keep cannot be recovered. There is no merge: the two boards are kept whole so you never end up with a half-merged mix of both.
</Warning>

<Note>
	If the two copies turn out to be identical, this prompt never appears, no matter how many times either side was saved.
</Note>

## When saving to your account keeps failing

Auto-save pushes to your account every 60 seconds while a strategy has unsaved changes. If the server refuses a save, the editor retries on a widening schedule and then stops, rather than failing over and over in the background.

When it stops, a banner appears above the canvas explaining why. The most common cause is a board with more shapes than a single save allows, in which case the banner names both your shape count and the limit. Deleting shapes to get back under the limit, or any other edit, immediately triggers a fresh attempt.

<Note>
	Your work is never lost when this happens - it stays saved on this device, and the banner clears as soon as a save succeeds. Being offline does not count against the retries.
</Note>
...
<Frame>
	<img
		src="/tools/battles-strategy/preview-1.webp"
...
- **Black Ops 6** - 2 player variants, 24 equipment tokens, and 5 generic markers: 31 tokens total.
- **Black Ops 7** - 2 player variants, 24 equipment tokens, and 5 generic markers: 31 tokens total.
- **Counter-Strike 2** - 2 player variants, 6 equipment tokens, and 5 generic markers: 13 tokens total.
- **League of Legends** - 2 player SVGs, 16 tactical default SVGs for objectives, vision, structures, and jungle, plus 172 champion WebPs: 190 tokens total. Tactical defaults appear before the champion portraits.

Switching the active game swaps the catalog automatically.

### Placing tokens

Drag a token from the panel onto the canvas, or click a token to arm it and then click on the canvas to place it. On mobile, tap a token then tap the canvas (no drag required). Player tokens are placed with a default color of red; recolor to blue or neutral from the **Properties** panel. You can also give a player token a jersey number from the same panel. Select any SVG token to expose its fill and outline slots in **Properties**. Champion portraits retain their authored image colors and cannot be recolored.
