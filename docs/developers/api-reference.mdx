...
<Note>
	The OpenAPI specification at `/api-reference/openapi.json` covers 30 paths: three match listing endpoints (user / team / org), two Twitch badge endpoints, eight league endpoints, the game-developer score and lifecycle endpoints, and the webhook management endpoints. Game-developer score and lifecycle endpoints (including forfeit, batch-scores, and game-originated create) are now documented in the spec; overlay endpoints and OAuth-proxy routes remain intentionally excluded.
</Note>
