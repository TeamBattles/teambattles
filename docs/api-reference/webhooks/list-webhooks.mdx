...
Webhook endpoints are per-creator - you only ever see the endpoints you created. That includes the endpoints in your key's derived scope (a personal key resolves to your `user` scope, a game-developer key to its bound `game`) plus any league-scoped endpoints you created. Endpoints created by anyone else are never returned.
