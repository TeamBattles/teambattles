...
const leagueIdentifier = "my-league";

const response = await fetch(`https://teambattles.gg/api/v1/leagues/${leagueIdentifier}`, {
	method: "POST",
	headers: {
...
The `{identifier}` path parameter accepts the league slug or Convex league ID.
