...
1. **Check widget is enabled** - Open the [editor](https://teambattles.gg/overlay/editor) and verify the widget is toggled on in the Widgets step.
2. **Check entity visibility** - Team Stats requires the selected team to be public (`profileVisibility: "public"`). Organization Stats requires the org's `isPublic` flag. User-scoped widgets require your profile visibility be non-private. The editor's Widgets step warns inline when the selected team or org will appear empty in OBS. See [Visibility & Privacy](/tools/battles-overlay/api-permissions).
3. **Check context selection** - Team Stats and Organization Stats require a team or organization to be selected in the picker under the widget's toggle in the Widgets step.

## Stale or Outdated Data

Stats on the overlay are not updating.

1. **Check refresh interval** - The default is 60 seconds. You can lower it (down to 30 seconds) in the editor's Go Live step for faster updates.
...
Also make sure the **Background opacity** setting in the editor's Style step is not set to 100%. An opacity of 70-85% typically works well for stream overlays.

## Overlay Too Small or Too Large

Widgets appear at the wrong size on stream.

1. **Adjust scale in the editor** - Use the scale slider in the editor's Style step to resize all widgets proportionally.
2. **Check OBS dimensions** - Set the Browser Source width to `1920` and height to `1080` (the recommended full-canvas size) for the intended layout. Other resolutions may cause scaling issues.
