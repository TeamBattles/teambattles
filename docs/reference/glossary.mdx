...
| Term                     | Definition                                                                                  |
| ------------------------ | ------------------------------------------------------------------------------------------- |
| **Account Bracket**      | A bracket saved to your TeamBattles account: counted against your tier's saved-bracket limit, openable from any device, shareable via a public link, and movable back to a single device to free a slot |
...
| **Bracket Visibility**   | Whether a saved bracket's share link opens for anyone else: **Private** (only you) or **Anyone with the link** (a read-only public view). Set in the Brackets tool's Save dialog; the link itself exists either way |
...
| **Device Bracket**       | A bracket stored only on the device you built it on, including its entrant images: it costs nothing against your saved-bracket limit and works signed out, but it is lost if you clear that browser's site data |
...
| **Move to device**       | Turning an account bracket back into a device bracket: its document and entrant images are copied to this device and the account copy is deleted, which frees a saved-bracket slot and invalidates its share link |
...
| **Project File (`.tbbrac`)** | The Brackets tool's own download format: the whole bracket plus the entrant logos that live only in your browser, so a bracket moves between browsers complete. Importing one restores those logos (each re-checked as if you had uploaded it); importing a plain `.json` bracket does not |
