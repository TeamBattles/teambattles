...
| **Webhook Event Catalog** | The set of event names a webhook can subscribe to - 83 events across families such as `match.*`, `score.*`, `team.*`, `organization.*`, and `league.*` (plus a `test.ping` test event). See the [Webhooks guide](/developers/webhooks) for the full catalog. |
