...
if TYPE_CHECKING:
    ...
    from .discover_get_response import DiscoverGetResponse
...
class DiscoverRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/orgs/discover{?limit*,search*}", path_parameters)
    
    async def get(self,request_configuration: Optional[RequestConfiguration[DiscoverRequestBuilderGetQueryParameters]] = None) -> Optional[DiscoverGetResponse]:
        """
        ...
        Returns: Optional[DiscoverGetResponse]
        """
        ...
        return await self.request_adapter.send_async(request_info, DiscoverGetResponse, error_mapping)
    
    def to_get_request_information(self,request_configuration: Optional[RequestConfiguration[DiscoverRequestBuilderGetQueryParameters]] = None) -> RequestInformation:
        """
        Returns a neutral, ranked discovery list of active, public, non-disabled organizations. Each result is a small summary (id, slug, name, tag, avatarUrl, visibility); `count` is the size of the returned page, not a total. Results are sorted featured organizations first, then by most wins, then newest. `limit` is 1-100 (default 50); ranking is computed over at most the first 1000 candidate organizations and there is no cursor pagination, so the response is a single ranked page. `search` filters by case-insensitive name substring and is only applied when it is at least 2 characters (shorter values are ignored). Unlike an in-app feed, results do not exclude organizations the API key owner belongs to and do not include membership or join-request flags. Requires orgs.profile:read.
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
    ...
    @dataclass
    class DiscoverRequestBuilderGetRequestConfiguration(RequestConfiguration[DiscoverRequestBuilderGetQueryParameters]):
        """
