...
if TYPE_CHECKING:
    ...
    from ....models.game_create_match_body import GameCreateMatchBody
    from ....models.game_create_match_response import GameCreateMatchResponse
...
class CreateRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/game/matches/create", path_parameters)

    async def post(self,body: GameCreateMatchBody, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[GameCreateMatchResponse]:
        """
        ...
        Returns: Optional[GameCreateMatchResponse]
        """
        if body is None:
        ...
        return await self.request_adapter.send_async(request_info, GameCreateMatchResponse, error_mapping)

    def to_post_request_information(self,body: GameCreateMatchBody, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> RequestInformation:
        """
        Creates an ACCEPTED + PUBLISHED match for the API key's bound game between two active teams in that game. Requires the game.lifecycle:read-write permission. Supply an optional `Idempotency-Key` request header to make retries safe: an identical key + body replays the original response, while the same key with a different body returns 409 error_idempotency_key_conflict.
        param body: Request body for creating a two-sided game-originated match. The match is created ACCEPTED + PUBLISHED for the key's bound game. Supports an optional Idempotency-Key header for safe retries.
        param request_configuration: Configuration for the request such as headers, query parameters, and middleware options.
