<?php

namespace TeamBattles\Sdk\Generated\Models;

use Microsoft\Kiota\Abstractions\Serialization\Parsable;
use Microsoft\Kiota\Abstractions\Serialization\ParseNode;
use Microsoft\Kiota\Abstractions\Serialization\SerializationWriter;

class ReportChatMessageResponse implements Parsable 
{
    /**
     * @var bool|null $alreadyReported True if the owner had already reported this message.
    */
    private ?bool $alreadyReported = null;
    
    /**
     * @var string|null $reportId Report ID (existing one if already reported).
    */
    private ?string $reportId = null;
    
    /**
     * @var bool|null $success The success property
    */
    private ?bool $success = null;
    
    /**
     * @var string|null $timestamp Response generation time (ISO 8601).
    */
    private ?string $timestamp = null;
    
    /**
     * Creates a new instance of the appropriate class based on discriminator value
     * @param ParseNode $parseNode The parse node to use to read the discriminator value and create the object
     * @return ReportChatMessageResponse
    */
    public static function createFromDiscriminatorValue(ParseNode $parseNode): ReportChatMessageResponse {
        return new ReportChatMessageResponse();
    }

    /**
     * Gets the alreadyReported property value. True if the owner had already reported this message.
     * @return bool|null
    */
    public function getAlreadyReported(): ?bool {
        return $this->alreadyReported;
    }

    /**
     * The deserialization information for the current model
     * @return array<string, callable(ParseNode): void>
    */
    public function getFieldDeserializers(): array {
        $o = $this;
        return  [
            'alreadyReported' => fn(ParseNode $n) => $o->setAlreadyReported($n->getBooleanValue()),
            'reportId' => fn(ParseNode $n) => $o->setReportId($n->getStringValue()),
            'success' => fn(ParseNode $n) => $o->setSuccess($n->getBooleanValue()),
            'timestamp' => fn(ParseNode $n) => $o->setTimestamp($n->getStringValue()),
        ];
    }

    /**
     * Gets the reportId property value. Report ID (existing one if already reported).
     * @return string|null
    */
    public function getReportId(): ?string {
        return $this->reportId;
    }

    /**
     * Gets the success property value. The success property
     * @return bool|null
    */
    public function getSuccess(): ?bool {
        return $this->success;
    }

    /**
     * Gets the timestamp property value. Response generation time (ISO 8601).
     * @return string|null
    */
    public function getTimestamp(): ?string {
        return $this->timestamp;
    }

    /**
     * Serializes information the current object
     * @param SerializationWriter $writer Serialization writer to use to serialize this model
    */
    public function serialize(SerializationWriter $writer): void {
        $writer->writeBooleanValue('alreadyReported', $this->getAlreadyReported());
        $writer->writeStringValue('reportId', $this->getReportId());
        $writer->writeBooleanValue('success', $this->getSuccess());
        $writer->writeStringValue('timestamp', $this->getTimestamp());
    }

    /**
     * Sets the alreadyReported property value. True if the owner had already reported this message.
     * @param bool|null $value Value to set for the alreadyReported property.
    */
    public function setAlreadyReported(?bool $value): void {
        $this->alreadyReported = $value;
    }

    /**
     * Sets the reportId property value. Report ID (existing one if already reported).
     * @param string|null $value Value to set for the reportId property.
    */
    public function setReportId(?string $value): void {
        $this->reportId = $value;
    }

    /**
     * Sets the success property value. The success property
     * @param bool|null $value Value to set for the success property.
    */
    public function setSuccess(?bool $value): void {
        $this->success = $value;
    }

    /**
     * Sets the timestamp property value. Response generation time (ISO 8601).
     * @param string|null $value Value to set for the timestamp property.
    */
    public function setTimestamp(?string $value): void {
        $this->timestamp = $value;
    }

}
